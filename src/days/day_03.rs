...
fn extract_mul_instructions<'a>(instructions: &'a str) -> impl Iterator<Item = Instruction> + 'a {
    PATTERN
        .captures_iter(instructions)
        .map(|capture| match capture.extract() {
            (_, [fst, snd]) => Instruction::Mul(
                fst.parse().expect("should be a valid integer"),
                snd.parse().expect("should be a valid integer"),
            ),
        })
}
fn extract_valid_instructions<'a>(instructions: &'a str) -> impl Iterator<Item = Instruction> + 'a {
